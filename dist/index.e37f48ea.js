...
const controlRecipes = async function() {
    try {
        ...
        if (!id) return;
        (0, _recipeViewJsDefault.default).displaySpinner();
        ...
        (0, _recipeViewJsDefault.default).displayError();
        console.error(err);
    }
};
const controlSearchResult = async function() {
    try {
        (0, _resultViewJsDefault.default).displaySpinner();
        // ---------------get search query-----------------------
...
const controlPagination = function(goPage) {
    // --------------display new results------------------------
    (0, _resultViewJsDefault.default).render(_modelJs.getSearchResultPage(goPage));
...
const controlAddRecipe = async function(newRecipe) {
    try {
        (0, _addRecipeViewJsDefault.default).displaySpinner();
        await _modelJs.uploadRecipe(newRecipe);
        console.log(_modelJs.state.recipe);
        //----------------display recipe----------------
        (0, _recipeViewJsDefault.default).render(_modelJs.state.recipe);
        //--------------display success message-------------
        (0, _addRecipeViewJsDefault.default).displayMsg();
        //----------------change url id------------------
        ...
        console.error(err);
        (0, _addRecipeViewJsDefault.default).displayError(err.message);
    }
};
const init = function() {
    (0, _bookmarkViewJsDefault.default).addHandlerDisplay(controlBookmark);
    (0, _recipeViewJsDefault.default).addHandlerDisplay(controlRecipes);
...
const state = {
    ...
    search: {
        ...
        query: "",
        page: 1,
        resultPage: (0, _config.RES_PER_PAGE)
    },
    bookmark: []
};
...
const uploadRecipe = async function(newRecipe) {
    try {
        const ingredient = Object.entries(newRecipe).filter((entry)=>entry[0].startsWith("ingredient") && entry[1] !== "").map((ing)=>{
            const ingArr = ing[1].replaceAll(" ", "").split(",");
            if (ingArr.length !== 3) throw new Error(" Wrong ingredient format. Please use the correat format! ");
...
const API_KEY = "2218e1ac-9569-4d15-8d1d-0e0f8ba4f182";
...
class recipeView extends (0, _viewDefault.default) {
    ...
    _errorMsg = "we can not find the recipe, please try again";
    _successMsg = "";
    ...
    _generateMarkupIngredient(ingred) {
      ...
      <li class="recipe__ingredient">
        ...
        <div class="recipe__quantity">${ingred.quantity ? ingred.quantity : ""}</div>
...
class View {
    ...
    update(data) {
        ...
        newElm.forEach((newElms, i)=>{
            ...
            if (!newElms.isEqualNode(curELms) && newElms.firstChild?.nodeValue.trim() !== "") curELms.textContent = newElms.textContent;
            if (!newElms.isEqualNode(curELms)) Array.from(newElms.attributes).forEach((attr)=>curELms.setAttribute(attr.name, attr.value));
        });
    }
    // ------------------display spinner-----------------
    displaySpinner() {
...
class ResultView extends (0, _viewDefault.default) {
    ...
    _errorMsg = "the recipes name is not correct for your query! please input the right name";
    _successMsg = "";
    _generateMarkup() {
...
class paginationView extends (0, _viewDefault.default) {
    ...
    _generateMarkup() {
        ...
        const pagesNum = Math.ceil(this._data.results.length / this._data.resultPage);
        // ----------in page 1, have other pages-------------
        if (curPage === 1 && pagesNum > 1) return `
      <button data-goto="${curPage + 1}" class="btn--inline pagination__btn--next">
        ...
        // ---------------in last page-----------------
        if (curPage === pagesNum && pagesNum > 1) return `
...
class AddRecipeView extends (0, _viewDefault.default) {
    _parentElm = document.querySelector(".upload");
...
},{"./View":"5cUXS","url:../../img/icons.svg":"loVOp","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["aD7Zm","aenu9"], "aenu9", "parcelRequiree88c")
